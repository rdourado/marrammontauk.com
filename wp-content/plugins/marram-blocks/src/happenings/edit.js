...
class HappeningsEdit extends Component {
	renderControl = () => {
				...
				<ol className={`${className}__list`}>
					{data.map(post => (
						<li key={post.id} className={`${className}__event`}>
							<span className={`${className}__date`}>{post.meta.happening_date}</span>
							<p
...
export default withSelect(select => ({
	posts: select('core').getEntityRecords('postType', 'happening'),
	categories: select('core').getEntityRecords('taxonomy', 'happening_category') || [],
}))(HappeningsEdit)
